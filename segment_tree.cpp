#include <bits/stdc++.h>
using namespace std;
#define int long long
#define mod 1000000007
#define vi vector<int>
#define vvi vector<vi>
#define pi pair<int, int>
#define vpi vector<pi>
#define vb vector<bool>
#define vvb vector<vb>
#define sz(a) a.size()
#define float double
#define pb push_back
#define ff first
#define ss second
#define all(c) c.begin(), c.end()
#define min3(a, b, c) min(c, min(a, b))
#define min4(a, b, c, d) min(d, min(c, min(a, b)))
#define per(i,a,b) for(int i=a;i>=b;i--)
#define rep(i,a,b) for(int i=a;i<b;i++)
#define endl "\n"
#define fastasfuck ios_base::sync_with_stdio(false), cin.tie(nullptr), cout.tie(nullptr);

typedef long long ll;
typedef unsigned long long ull;
typedef long double lld;

#ifndef ONLINE_JUDGE
#define debug(x) cerr << #x <<" "; _print(x); cerr << endl;
#else
#define debug(x)
#endif

void _print(int t) {cerr << t;}
void _print(string t) {cerr << t;}
void _print(char t) {cerr << t;}
void _print(lld t) {cerr << t;}
void _print(double t) {cerr << t;}
void _print(ull t) {cerr << t;}

template <class T, class V> void _print(pair <T, V> p);
template <class T> void _print(vector <T> v);
template <class T> void _print(set <T> v);
template <class T, class V> void _print(map <T, V> v);
template <class T> void _print(multiset <T> v);
template <class T, class V> void _print(pair <T, V> p) {cerr << "{"; _print(p.ff); cerr << ","; _print(p.ss); cerr << "}";}
template <class T> void _print(vector <T> v) {cerr << "[ "; for (T i : v) {_print(i); cerr << " ";} cerr << "]";}
template <class T> void _print(set <T> v) {cerr << "[ "; for (T i : v) {_print(i); cerr << " ";} cerr << "]";}
template <class T> void _print(multiset <T> v) {cerr << "[ "; for (T i : v) {_print(i); cerr << " ";} cerr << "]";}
template <class T, class V> void _print(map <T, V> v) {cerr << "[ "; for (auto i : v) {_print(i); cerr << " ";} cerr << "]";}
template <typename typC>
istream &operator>>(istream &cin, vector<typC> &a)
{
	for (auto &x : a)
		cin >> x;
	return cin;
}
int n, q;
vector<int>a, seg, lazy;
//Range is [l,r) or [l, r-1];
void update(int idx, int l, int r, int val) {
	seg[idx] += val; // for sum query seg[idx] += (r - l)*val;
	lazy[idx] += val;
}
void shift(int idx, int l, int r) {
	if (lazy[idx] == 0) {
		return;
	}
	int mid = l + (r - l) / 2;
	update(2 * idx, l, mid, lazy[idx]);
	update(2 * idx + 1, mid, r, lazy[idx]);
	lazy[idx] = 0;
}
void build(int idx = 1, int l = 0, int r = n) {
	if (r - l == 1) {
		seg[idx] = a[l];
		return;
	}
	int mid = l + (r - l) / 2;
	build(2 * idx, l, mid);
	build(2 * idx + 1, mid, r);
	seg[idx] = min(seg[idx * 2], seg[idx * 2 + 1]);
}
const int inf = 1e18;
int query(int x, int y, int idx = 1, int l = 0, int r = n) {
	if (l >= y || r <= x) {
		return inf;
	}
	if (l >= x  && r <= y) {
		return seg[idx];
	}
	int mid = l + (r - l) / 2;
	shift(idx, l, r);
	int leftRes = query(x, y, 2 * idx, l, mid);
	int rightRes = query(x, y, 2 * idx + 1, mid, r);
	return min(leftRes, rightRes);
}
void modify(int x, int y, int val, int idx = 1, int l = 0, int r = n) {
	if (l >= y || r <= x) {
		return;
	}
	if (l >= x  && r <= y) {
		update(idx, l, r, val);
		return;
	}
	int mid = l + (r - l) / 2;
	shift(idx, l, r);
	modify(x, y, val, 2 * idx, l, mid);
	modify(x, y, val, 2 * idx + 1, mid, r);
	seg[idx] = min(seg[2 * idx], seg[2 * idx + 1]);
}
void soluchan()
{
	cin >> n >> q;
	a.resize(n);
	seg.resize(4 * n + 1);
	lazy.resize(4 * n + 1);
	cin >> a;
	build();
	for (int i = 0; i < q; i++) {
		int type;
		cin >> type;
		if (type == 1) {
			int x, y, val;
			cin >> x >> y >> val;
			modify(x, y, val);
		}
		else if (type == 2) {
			int x, y;
			cin >> x >> y;
			cout << query(x, y) << endl;
		}
	}
}

int32_t main() {
#ifdef HUEHUEHUE
	freopen("input.txt", "r", stdin);
	freopen("output.txt", "w", stdout);
	freopen("error.txt", "w", stderr);
#endif
	int t = 1;
	// cin >> t;
	while (t--) {
		soluchan();
	}
	return 0;

}

